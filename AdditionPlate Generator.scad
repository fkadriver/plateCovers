...
svg_horizontal_scroll = "Horizontal_scroll.svg";         // top/bottom center scrollwork
...
module horizontal_scroll() {
	...
	scroll_cy  = face_y - scroll_w / 2 -2; // right edge at face boundary
    translate([0,scroll_cy,plate_thickness])
		linear_extrude(height=bead_r, scale=scroll_scale)
			resize([scroll_len, scroll_w])
                import(svg_horizontal_scroll,center=true);
    translate([0,-scroll_cy,plate_thickness])
		linear_extrude(height=bead_r, scale=scroll_scale)
			resize([scroll_len, scroll_w])
                import(svg_horizontal_scroll,center=true);
