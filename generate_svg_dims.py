...
def main():
    ...
    lines = [
        ...
        "// SVG bounding box origin (x, y) and dimensions (w, h); all in mm.",
        "// Re-run generate_svg_dims.py after modifying any SVG file.",
        "",
    ]

    print(f"{'File':<30} {'x (mm)':>10} {'y (mm)':>10} {'Width (mm)':>10} {'Height (mm)':>10}")
    print("-" * 72)
    ...
    for svg_file in svg_files:
        ...
        lines.append(f"svg_{stem}_x = {x:.4f};  // {svg_file} — bbox min-x (mm)")
        lines.append(f"svg_{stem}_y = {y:.4f};  // {svg_file} — bbox min-y (mm)")
        lines.append(f"svg_{stem}_w = {w:.4f};  // {svg_file} — width  (mm)")
        lines.append(f"svg_{stem}_h = {h:.4f};  // {svg_file} — height (mm)")
        print(f"{svg_file:<30} {x:>10.4f} {y:>10.4f} {w:>10.4f} {h:>10.4f}")
